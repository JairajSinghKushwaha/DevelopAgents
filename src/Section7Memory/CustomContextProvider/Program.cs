...
sealed class EmployeeProfileProvider : AIContextProvider
{
    ...
    public EmployeeProfileProvider(IChatClient chatClient) : base(default, default)
    {
        _sessionState =
            new ProviderSessionState<EmployeeProfile>(_ => new EmployeeProfile(), this.GetType().Name);
    ...
    protected override async ValueTask StoreAIContextAsync(InvokedContext context, CancellationToken cancellationToken = default)
    {
        ...
        if ((profile.EmployeeName is null || profile.Department is null) && context.RequestMessages.Any(x => x.Role == ChatRole.User))
        {
            ...
            new ChatOptions() { Instructions = "Extract the user's name and corporate department for the message if present. If not present, return nulls." },
...
public class Program
{
    public static async Task Main()
    {
        ...
        {
            Name = "CorporateGuide",
            ChatOptions = new() { Instructions = "Yes are a friendly internal corporate assistent." },
        ...
        Console.WriteLine(await agent.RunAsync("What is the company policy on the remote work?", session));
        Console.WriteLine(await agent.RunAsync("My name is Jairaj and U work in IT Deparment.", session));

        JsonElement serializedSession = await agent.SerializeSessionAsync(session);
        Console.WriteLine("\n--- Simulate a New Day (Deserialize Session) ---");
        ...
        // Accessing the strongly typed memory direclty from code
        ...
        Console.WriteLine($"Extract Name: {profile?.EmployeeName}");
        Console.WriteLine($"Extract Department: {profile?.Department}");
    }
}