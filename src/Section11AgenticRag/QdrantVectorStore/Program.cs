...
var embeddingClientName = Environment.GetEnvironmentVariable("AZURE_OPENAI_EMBEDDING_NAME") ?? "text-embedding-3-small";
...
// Initialize the Chat Client (The Agent's Brain)
...
// Seed the collection with sample ADR records
...
// Generate embaddings and upsert records
...
// Configure the TextSearchProvider options for RAG behavior
TextSearchProviderOptions textSearchProviderOptions = new()
{
    SearchTime = TextSearchProviderOptions.TextSearchBehavior.BeforeAIInvoke
};

// Create the Vector Search Adaptor
async Task<IEnumerable<TextSearchProvider.TextSearchResult>> VectorSearchAdaptor(string query, CancellationToken cancellationToken)
{
    ...
    // Search the Qdrant vector store for sementically similar ADRs(top 3 results)
...
// Initialize the Agent with the Qdrant-backed Rag capability
AIAgent architectureAgent = chatClient.AsAIAgent(new ChatClientAgentOptions()
{
    ...
    AIContextProviders = [new TextSearchProvider(VectorSearchAdaptor, textSearchProviderOptions)]
});

Console.WriteLine("--- Enterprise Architecture Swarm Online ---\n");

// The active researcher loop begins
string query = "Why did we choose gRPC over REST for the internal Microservice Communication in 2024?";
...
AgentResponse agentResponse = await architectureAgent.RunAsync(query);
